...
const Navbar = () => {
  ...
  return (
    <div className=" bg-black px-4 flex justify-between items-center w-full h-20 text-white fixed">
      <div>
        <h1 className=" text-5xl font-signature ml-2">Omid Portfolio</h1>
      </div>
      <ul className="hidden md:flex">
        {links.map(({ id, link }) => (
          <li
            key={id}
            className=" px-4 cursor-pointer capitalize hover:underline font-medium text-gray-500 hover:scale-105 duration-200"
          >
            {link}
          </li>
        ))}
